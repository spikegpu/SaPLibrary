...
public:
	...
	template <typename T>
	void operator() (const string& s, T  item) {
		m_o << "\"" << s << "\":" << item << ",";
	}
};
...
private:
	...
	string m_fileSol;
	int    m_numPart;
	spike::Options m_opts;
public:
	MatrixTest(const string& testName, const string& testProjectName, const string& fileMat, const string& fileRhs, const string& fileSol, int numPart, const spike::Options& opts)
	: BaseTest(testName, testProjectName),
	  m_fileMat(fileMat), m_fileRhs(fileRhs), m_fileSol(fileSol),
	  m_numPart(numPart), m_opts(opts)
	{
		m_execTime = 0.0;
	}
...
// MAIN
// -----------------------------------------------------------------------------
bool MatrixTest::execute()
{
	// Set up the problem to be solved.
	string&         fileMat = m_fileMat;
	string&         fileRhs = m_fileRhs;
	int&            numPart = m_numPart;
	spike::Options& opts = m_opts;
	...
	// Dimension
	addMetric("N", (int)(A.num_rows));
	// No. of non-zeros
	// addMetric("NNZ", (int)(A.num_entries));
	// SPD
	...
	try {
		...
		addMetric ("Solves", "OoM (in setup stage)");
		// Enable Diagonal boosting?
		addMetric ("Bstng", opts.safeFactorization);
		...
		switch(se.reason()) {
			case spike::system_error::Zero_pivoting:
				addMetric ("Solves", "ZPiv");
				break;
			case spike::system_error::Matrix_singular:
				addMetric ("Solves", "MatSing");
				break;
			...
			case spike::system_error::Negative_MC64_weight:
				...
				break;
			default:
				addMetric ("Solves", "Unknown error");
				break;
		}
		// Enable Diagonal boosting?
		addMetric ("Bstng", opts.safeFactorization);
...

	{
		...
		// Half-bandwidth
		addMetric("K", (int)(stats.bandwidth));
		// Actual NNZ after LU
		addMetric("FRate", 1.0 * stats.actual_nnz / (A.num_rows * ((stats.bandwidth << 1) + 1)));
		// nuKf
		addMetric("nuKf", stats.nuKf);

		// Reason why cannot solve (for unsuccessful solving only)
		if (solveSuccess)
			addMetric ("Solves", "OK");
		else
			addMetric ("Solves", "NConv");
		// Enable Diagonal boosting?
		addMetric ("Bstng", opts.safeFactorization);
		...
		addMetric("T-DTransf", stats.time_transfer);
		// Time to assemble banded and off-diagonal matrices on CPU
		addMetric("T-Asmbl", stats.time_cpu_assemble);
		...
		if (opts.ilu_level >= 0) {
			if (opts.variableBandwidth)
				...
				addMetric("LU-M", "ILUULT");
		} else
			addMetric("LU-M", "LU");
		// Fill-in factor
		addMetric("Fill-in", opts.ilu_level);
		// Number of partitions
		addMetric("NPrtns", numPart);
		// Time to extract all off-diagonal matrices on GPU
		addMetric("T-BC", stats.time_offDiags);
		// Time for banded LU and UL
		addMetric("T-LU", stats.time_bandLU + stats.time_bandUL);
		// GFLOPS of banded LU
		addMetric("GFlps-LU", stats.flops_LU);
		// Time to assemble off-diagonal matrics on GPU (including the solution of multi-RHS)
		addMetric("T-SPK", stats.time_assembly);
		// Time for full LU on reduced matrices
		addMetric("T-LUrdcd", stats.time_fullLU);
		// Total time for setup
		addMetric("T-PreP", stats.timeSetup);
		// Krylov method
		{
			...
			addMetric("Kry-M", prec.data());
		}
		// Number of iterations to converge
		addMetric("nItrs", stats.numIterations);
		// Total time for Krylov solve
		addMetric("T-Kry", stats.timeSolve);
		// Total amount of time
		addMetric("T-total", stats.timeSetup + stats.timeSolve);
		m_execTime = stats.timeSetup + stats.timeSolve;
...
int main(int argc, char **argv) {
	...
	// Name of matrix
	{
		...
		if (j != std::string::npos)
			fileMat = fileMat.substr(0, j);
	}
	string projectName = "SpikeGPU";
	MatrixTest matrix_test(fileMat, projectName, ori_fileMat, fileRhs, fileSol, numPart, opts);
	matrix_test.run();
